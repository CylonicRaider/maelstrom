...
class Maelstrom {
  ...
  /* Return values to template the file at path with */
  _getTemplateValues(path) {
    return {};
  }
  ...
  makeServer() {
    ...
    const fc = this.router.getStatic(/\/static\/(.*)/, '$1', 'static');
    // Dynamics
    fc.callback = templater.makeFileCacheCallback('text/html',
      this._getTemplateValues.bind(this));
    // Catch-all routes
