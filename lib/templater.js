...
function compile(input) {
  function checkConstSubs(part, tokens) {
    if (tokens.length != 1)
      throw new Error('Invalid substitution (need 1 token): ' + part);
  }
  function checkVarSubs(part, tokens) {
    if (tokens.length != 2)
  ...
  function checkStack(part, expected) {
