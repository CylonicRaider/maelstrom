...
class CacheNode extends EventEmitter {
  ...
  /* Load this node with the result of invoking source
   *
   * The result is dispatched to the loadDirect() or loadStream() methods
   * depending on the return value of source, which is stored for later
   * use in revalidation. */
  load(source) {
    const input = source();
    if (typeof input == 'string') {
      this.loadDirect(input, source);
    } else if (input instanceof Buffer) {
      this.loadDirect(input, source);
    } else if (input instanceof stream.Readable) {
      this.loadStream(input, source);
    } else {
      process.nextTick(() => {
   ...
   * Returns the current CacheNode instance. */
  loadFile(path) {
    return this.load(() => fs.createReadStream(path));
  }
...
class FileCache {
   ...
   * If not null, the callback property is invoked on every (successful)
   * request, with the request, the response, and the data to be sent as
   * arguments. If its return value is not null (or undefined), the data to
   * be sent are replaced with the return value. In that case, the callback
   * should also remove (or suitably replace) the response's ETag and Cache-
   * Control headers.
  ...
  _handle(req, resp, fsPath) {
    ...
    item.done((err, data) => {
      ...
      if (this.callback != null) {
        // Allow injected dependency to modify the response
        const res = this.callback(req, resp, data);
        if (res != null) data = res;
      }
      const finalETag = resp.getHeader('ETag');
      if (finalETag && req.headers['if-none-match'] == finalETag) {
        resp.statusCode = 304;
        resp.end();
      } else {
