...
const { stringResponder, FileCache } = require('./filecache.js');
const { literalRE, template } = require('./util.js');
...
class Router {
  ...
  /* Install a handler serving a fixed response
   *
   * method and path are the same as the corresponding route() arguments;
   * further arguments are passed on to the stringResponder() function; see
   * there for details. */
  ...
  /* Install a handler serving a fixed response for GET-s
   *
   * Equivalent to routeString('GET', path, ...args); see there. */
  getString(path, ...args) {
    this.routeString('GET', path, ...args);
  }
