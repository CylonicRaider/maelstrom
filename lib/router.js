...
class Router {
   ...
   *
   * Only files matching path are served; template is used to transform them
   * into filesystem path as detailed in the constructor of FileCache.
   * Returns the FileCache instance created.
   */
  getStatic(path, template) {
    const ret = new FileCache(path, template)
